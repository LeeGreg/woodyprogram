...
* 生产者-传输通道-消费者
  ...
  * 阻塞队列
    ...
    * BlockingQueue是一种线程安全的阻塞队列，实现类
      ...
      * LinkedBlockingQueue
        * 构造函数中指定容量时是有界对类，否则是无界队列，内部的链表存储空间动态分配，put和take操作可能增加垃圾回收的负担，但是使用了两个显示锁（putLock、takeLock）降低了锁争用
...
* Executor是一个顶层接口，只声明了一个⽅法`void execute(Runnable)`，空闲时无法主动释放内部工作者线程所占用的资源
